...
import { PopupsSchema, pickActiveSorted, type Popup } from "@/lib/popup";
...
// its popups. Response shape: { popups } — the active list, priority-sorted —
// plus a legacy { popup } (the top one) for back-compat.
...
export async function GET() {
  ...
  const active = pickActiveSorted(popups, new Date());
  // `popups` is the prioritized list the client picks from (it applies the
  // per-visitor frequency/seen check). `popup` is kept for any client still
  // expecting a single popup during a rolling deploy.
  return NextResponse.json({ popups: active, popup: active[0] ?? null });
}
